...
class DataModel:
    ...
    def initialize(self, init_data: List[List[int]]):
        ...
        for row in range(3):
            sgroups = []
            for col in range(3):
                data = [self.cells[3 * row + i][3 * col + j] for j in range(3) for i in range(3)]
        ...
        for group_index in range(3):
            ...
            for cell_index in range(3):
                data = [self.cells[i * 3 + j][group_index * 3 + cell_index] for j in range(3) for i in range(3)]
        ...
        for group_index in range(3):
            hgroups = []
            for cell_index in range(3):
                data = [self.cells[group_index * 3 + cell_index][i * 3 + j] for j in range(3) for i in range(3)]
                hgroups.append(HorizontalGroupModel(data))
            self.horizontal_groups.append(hgroups)
        ## cross groups
        self.cross_groups = []
        cgroups = []
        data = [self.cells[i + 3 * j][i + 3 * j] for j in range(3) for i in range(3)]
        cgroups.append(CrossGroupModel(data))
        data = [self.cells[i + 3 * j][8 - (i + 3 * j)] for j in range(3) for i in range(3)]
        cgroups.append(CrossGroupModel(data))
